# Handoff: rearchitecting detection (archived)

> **Archived record from the retired `src/core/` lineage.** Preserved verbatim
> from `claude/tuninator-code-review-q5yzz2` @ `053526b`. **Nothing here
> describes the shipping detector**; the paths, scripts and commands below
> (`src/core/`, `scripts/strict.ts`, `tests/fixtures.test.ts`) do not exist on
> `main`. It is kept because §2 is the measurement that motivated the current
> architecture: a still-ringing previous note dominates the new one, so a
> monophonic estimator that reports the loudest periodicity is *correct* and
> still marked wrong. The answer to that finding is the voices-vs-Notes
> distinction and overlapping Notes in `src/engine/tracker/` — the thing the
> single-active-event `EventTracker` described here could not represent.
>
> The lineage's own accuracy, measured against the full corpus on the same
> ground truth, is in `docs/DETECTION-FINDINGS.md` ("The retired `src/core/`
> lineage, scored on the held-out corpus") and DECISION-023. Summary: it wins
> on the five takes it was tuned on and loses badly everywhere else.
