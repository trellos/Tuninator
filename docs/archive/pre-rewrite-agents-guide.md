# Contributor guide for the retired `src/core/` architecture (archived)

> **Archived. Superseded by the repository-root `AGENTS.md`.** Preserved from
> `claude/tuninator-orchestration-agent-phcm0m` @ `67d2fa2`, which branched
> before the streaming-recognizer rewrite and never carried it.
>
> **Do not follow this as guidance.** It documents `src/core/`, `policy.ts` and
> the single-active-event `EventTracker`, none of which exist on `main`, and it
> predates the decision-logging protocol entirely. It is kept for the design
> reasoning it records about the pre-rewrite lane structure and purity rule —
> the ancestor of the `src/engine/**` isolation invariant that replaced it.
