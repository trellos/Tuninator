...
# Only for the model card's own example command, which names the q4_0 file;
# every measurement reads the f32 file.
fetch "https://huggingface.co/cstr/hft-transformer-GGUF/resolve/$HF_REV/hft-transformer-q4_0.gguf" \
  "$OUT/hf/hft-transformer-q4_0.gguf" c72a56b9ceaca3091b04ca48bb52e882bf2f44a8ce603bc9cb64224e0c987dc3
...
# CMake's Makefile generator leaves dependency stamp files named
# `compiler_depend.ts` in every target directory. The root tsconfig includes
# training/, so a root `tsc --noEmit` would try to compile them; they are
# regenerated by the next build and nothing else reads them.
find "$B" -name compiler_depend.ts -delete
